...
import { loginBiometria, temBiometriaNesteAparelho, temAutenticadorPlataforma, getBioEmail } from '../lib/webauthn';
...
export default function Login({ onLogin, onSession }: Props) {
  ...
  const [enrollEmail, setEnrollEmail] = useState<string | null>(null);
  const [temAtalho, setTemAtalho] = useState(temBiometriaNesteAparelho());
  const navigate = useNavigate();

  useEffect(() => { setTemAtalho(temBiometriaNesteAparelho()); }, []);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Estilo banco: apos login com senha, oferecer salvar a biometria.
      const email = form.email.toLowerCase().trim();
      const podeOferecer = await temAutenticadorPlataforma()
        && !temBiometriaNesteAparelho(email)
        && sessionStorage.getItem('bio_prompt_skipped') !== email;
      if (podeOferecer) {
    ...
    <div className="min-h-[calc(100vh-64px)] flex items-center justify-center px-4 py-12 bg-gray-50">
      <div className="w-full max-w-md">
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
          ...
          {/* Atalho de biometria (estilo banco): aparece quando este aparelho ja tem digital salva */}
          {temAtalho && (
            <div className="mb-6">
              <button
                type="button"
                ...
                disabled={bioLoading}
                className="w-full py-3.5 bg-gradient-to-r from-brand-600 to-brand-700 text-white font-semibold rounded-xl hover:from-brand-700 hover:to-brand-800 transition-colors disabled:opacity-60 flex items-center justify-center gap-2.5 shadow-sm shadow-brand-600/20"
              >
              ...
              {bioEmail && <p className="text-center text-xs text-gray-400 mt-2">{bioEmail}</p>}
              <div className="flex items-center gap-3 mt-5">
                <div className="flex-1 h-px bg-gray-100" />
                <span className="text-xs text-gray-400">ou use a senha</span>
                <div className="flex-1 h-px bg-gray-100" />
              </div>
            </div>
          )}
      ...
      {enrollEmail && (
        <BiometricEnrollModal email={enrollEmail} onDone={() => { setEnrollEmail(null); navigate('/dashboard'); }} />
      )}
    </div>
  );
}
