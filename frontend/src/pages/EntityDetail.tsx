...
function DataRow({ label, value }: { label: string; value?: string | null }) {
  ...
  return (
    <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-0 py-2.5 border-b border-gray-50 last:border-0">
      <span className="sm:w-48 text-xs font-medium text-gray-400 uppercase tracking-wide flex-shrink-0">{label}</span>
...
export default function EntityDetail() {
  ...
  const cd = !isPerson ? detail as CompanyDetail : null;
      ...
      <div className="bg-white rounded-xl border border-gray-100 p-6 mb-5">
        <div className="flex flex-col sm:flex-row sm:items-center gap-4">
          <div className={`w-16 h-16 rounded-2xl flex items-center justify-center flex-shrink-0 ${
            isPerson ? 'bg-blue-50' : 'bg-purple-50'
          }`}>
            {isPerson ? (
              <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke={isPerson ? '#2563eb' : '#7c3aed'} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              <span className={`text-xs px-2.5 py-1 rounded-full font-semibold ${
                isPerson ? 'bg-blue-50 text-blue-700' : 'bg-purple-50 text-purple-700'
              }`}>
          ...
          >
            ...
            {!isPerson && cd && (
              <div>
                ...
                <DataRow label="Capital Social" value={cd.share_capital ? `R$ ${cd.share_capital.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}` : null} />
        ...
        <div className="space-y-5">
          ...
          >
            {addresses.length === 0 ? (
              ...
              <div className="space-y-3">
                {addresses.map((addr) => (
                  <div key={addr.id} className={`text-sm rounded-lg p-3 ${addr.is_current ? 'bg-brand-50 border border-brand-100' : 'bg-gray-50'}`}>
                    {addr.is_current && (
          ...
          >
            {phones.length === 0 && emails.length === 0 ? (
              <p className="text-sm text-gray-400 text-center py-2">Sem contatos.</p>
            ) : (
