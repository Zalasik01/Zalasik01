...
export default function Search() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="mb-8">
        ...
        {q && (
          <p className="text-xs text-gray-400 mt-2 ml-1">
            Dica: para busca direta na Receita Federal, digite o CNPJ completo (ex: 11.222.333/0001-81)
          </p>
        )}
      </div>
        ...
        <div className="flex items-center justify-center py-20">
          <div className="flex flex-col items-center gap-3">
            ...
            <span className="text-sm text-gray-500">
              {isCNPJ(q) ? 'Consultando Receita Federal...' : 'Consultando registros...'}
            </span>
          </div>
        </div>
      )}
      ...
      {!loading && !error && (
        <>
          {/* CNPJ direct result from Receita Federal */}
          {cnpjResult && (
            <div>
              <div className="flex items-center gap-2 mb-4">
                <span className="text-sm text-gray-500">
                  Resultado direto da{' '}
                  <span className="font-semibold text-gray-700">Receita Federal</span>
                </span>
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      {cnpjResult.trade_name && (
                        <p className="text-xs text-gray-400 mt-0.5">{cnpjResult.trade_name}</p>
                      )}
                      <div className="flex items-center gap-3 mt-1.5">
                        <span className="text-xs font-mono text-gray-400 bg-gray-50 px-2 py-0.5 rounded">
                          CNPJ: {cnpjResult.document}
                        </span>
                        {cnpjResult.secondary_date && (
                          <span className="text-xs text-gray-400">
                            Fund.: {new Date(cnpjResult.secondary_date).toLocaleDateString('pt-BR')}
                          </span>
                        )}
                      </div>
                    </div>
                    ...
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex items-center gap-3 min-w-0">
                        <div className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 ${
                          entity.type === 'person' ? 'bg-blue-50 text-blue-600' : 'bg-purple-50 text-purple-600'
                        }`}>
                        ...
                        <div className="min-w-0">
                          ...
                          <div className="flex items-center gap-3 mt-1.5">
                            ...
                            {entity.secondary_date && (
                              <span className="text-xs text-gray-400">
                                {entity.type === 'person' ? 'Nasc.:' : 'Fund.:'}{' '}
                                {new Date(entity.secondary_date).toLocaleDateString('pt-BR')}
                              </span>
                            )}
                          </div>
                        </div>
          ...
          {!cnpjResult && results.length === 0 && q && !loading && (
            <div className="text-center py-20">
              ...
              <p className="text-gray-500 text-sm">
                Para buscar por empresa, digite o CNPJ completo para consulta direta na Receita Federal.
              </p>
            </div>
          )}
